...
const { sortBy, sortDir, setSort, sortIcon } = useTableSort('budget_total', 'desc')
...
const filtered = computed(() => {
  ...
  return q
    ? props.rows.filter(d => String(d.department ?? '').toLowerCase().includes(q))
    : props.rows
})
...
const SORT_ACCESSORS = {
  department:      d => String(d.department ?? '').toLowerCase(),
  budget_total:    bget,
  budget_fund_101: f101,
  budget_fund_164: f164,
  budget_fund_161: f161,
  budget_fund_163: f163,
}
...
const normalizeSortValue = value => {
  ...
  return Number.isFinite(numeric) && text !== ''
    ? numeric
    : text.toLowerCase()
}

const sortedRows = computed(() => {
  const accessor = SORT_ACCESSORS[sortBy.value] ?? (d => d?.[sortBy.value] ?? 0)
  ...
  <SectionCard
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-[13px]">
        ...
        <tbody>
          <template v-for="(d, i) in sortedRows" :key="d.department + '_' + year">
        ...
        <tfoot v-if="sortedRows.length > 0">
          <tr class="border-t-2 border-gray-200 bg-[#0D2137]/[0.02]">
            ...
            <td v-for="col in COLUMNS" :key="col.key" class="px-6 py-3.5 text-right font-mono font-extrabold text-[#0D2137]">
              {{ php(sortedRows.reduce((s, d) => s + col.get(d), 0)) }}
            </td>
          </tr>
        </tfoot>
      </table>
