  ...
  <AppLayout>
    ...
    <div class="space-y-6 max-w-3xl">
      <div class="flex items-center justify-between">
        ...
        <button class="bg-[#022C22] text-white text-sm font-semibold px-4 py-2 rounded-xl hover:bg-[#168A4A] transition-colors flex items-center gap-2">
            ...
            <tr v-for="user in users" :key="user.email" class="border-b border-gray-50 hover:bg-[#F8FAFF] transition-colors">
              <td class="px-6 py-3.5">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#168A4A] to-[#047857] flex items-center justify-center text-white text-xs font-bold shrink-0">
                    {{ user.name[0] }}
                  </div>
                  <div>
              ...
              <td class="px-6 py-3.5">
                <span :class="[
                  ...
                  user.role === 'Admin' ? 'bg-[#022C22] text-white' : 'bg-gray-100 text-gray-600'
                ]">{{ user.role }}</span>
              </td>
              ...
              <td class="px-6 py-3.5 text-right">
                <button class="text-xs text-[#168A4A] hover:underline">Edit</button>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            <p class="font-semibold text-gray-700 text-sm flex items-center gap-2">
              <span class="w-5 h-5 rounded bg-[#022C22] inline-flex items-center justify-center text-white text-[10px]">A</span>
              Admin
            </p>
