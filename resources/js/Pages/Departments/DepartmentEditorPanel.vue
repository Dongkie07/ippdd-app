...
defineProps({
  ...
  selectedOfficeHistoricalName: { type: String, default: '' },
  funds: { type: Array, default: () => [] },
  formTotal: { type: Number, default: 0 },
  saving: { type: Boolean, default: false },
})

defineEmits(['close', 'submit', 'select-office'])
      ...
      <div class="relative z-10 w-full max-w-md bg-white shadow-2xl flex flex-col h-full">
        ...
        <div class="flex-1 overflow-y-auto px-7 py-6 space-y-5">
          <div class="rounded-2xl border border-[#DDEDE3] bg-[#F8FCF9] p-4">
            <label class="block text-[10px] font-extrabold uppercase tracking-widest text-gray-400 mb-1.5">Office Identity</label>
            <select
              ...
              @change="$emit('select-office', $event.target.value)"
            >
              <option value="">Manual / no linked office yet</option>
              <option v-for="office in officeOptions" :key="office.id" :value="office.id">
                {{ office.acronym ? `${office.acronym} · ` : '' }}{{ office.current_name }}
              </option>
            </select>

            <div v-if="selectedOffice" class="mt-3 grid gap-2 rounded-xl bg-white p-3 text-[11px] text-gray-500 ring-1 ring-[#DDEDE3]">
              <div class="flex items-center justify-between gap-3">
                <span class="font-bold text-gray-400">Latest name</span>
                <span class="text-right font-extrabold text-[#064E3B]">{{ selectedOffice.current_name }}</span>
              </div>
              <div class="flex items-center justify-between gap-3">
                <span class="font-bold text-gray-400">Name for FY {{ form.year }}</span>
                <span class="text-right font-extrabold text-[#168A4A]">{{ selectedOfficeHistoricalName }}</span>
              </div>
            </div>
          </div>

          <div>
            <label class="block text-[10px] font-extrabold uppercase tracking-widest text-gray-400 mb-1.5">Department / Office Name Used This Year *</label>
            <input
              v-model="form.department"
              type="text"
              placeholder="e.g. Planning and Resource Management Office"
              class="w-full px-3.5 py-2.5 text-[13px] border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#064E3B]/20 text-gray-700 bg-white"
            />
            <p class="mt-1.5 text-[11px] font-medium text-gray-400">
              This can stay as the old yearly name while reports still group it under one stable office identity.
            </p>
          </div>
