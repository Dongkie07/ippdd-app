...
<template>
  ...
  <div class="rounded-2xl border overflow-hidden transition-all duration-300"
    ...
    <button @click="toggle"
      ...
      :class="isOpen ? 'bg-gradient-to-r from-[#064E3B] to-[#1A3A5C]' : 'bg-white hover:bg-gray-50'">

      <div class="flex items-center gap-3">
        <!-- Gemini spark icon -->
        <div :class="['w-8 h-8 rounded-xl flex items-center justify-center transition-all',
          isOpen ? 'bg-[#C9A84C]/20 border border-[#C9A84C]/30' : 'bg-[#064E3B]/[0.06] border border-[#064E3B]/10']">
          <svg class="w-4 h-4" :class="isOpen ? 'text-[#C9A84C]' : 'text-[#064E3B]'"
            viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2L9.5 9.5 2 12l7.5 2.5L12 22l2.5-7.5L22 12l-7.5-2.5L12 2z"/>
          </svg>
        </div>
        <div class="text-left">
          <p :class="['text-[13px] font-extrabold tracking-tight', isOpen ? 'text-white' : 'text-[#064E3B]']">
      ...
      <div v-if="isOpen" class="bg-white">
        ...
        <div v-else-if="rawText" class="px-6 py-5">
          ...
          <div v-if="isDone" class="flex items-center justify-between mt-5 pt-4 border-t border-gray-100">
            ...
            <button @click="retry"
              class="flex items-center gap-1.5 text-[11px] font-bold text-[#064E3B] hover:text-[#C9A84C] transition-colors">
...
:deep(.ai-h1) { font-size: 15px; font-weight: 800; color: #064E3B; margin: 16px 0 8px; }
:deep(.ai-h2) { font-size: 14px; font-weight: 800; color: #064E3B; margin: 14px 0 6px; padding-bottom: 4px; border-bottom: 2px solid #F3F4F6; }
:deep(.ai-h3) { font-size: 13px; font-weight: 700; color: #0F766E; margin: 12px 0 4px; }
